...
public:
	...
	bool is_semiconnected() {
		Graph cg = component_graph();
		size_t SCC_SIZE = SCC.size();
		std::vector<std::vector<bool>> is_connected(SCC_SIZE, std::vector<bool>(SCC_SIZE, false));
		for (auto i = 0; i < cg.adj.size(); ++i) {
			for (auto j = 0; j < cg.adj[i].size(); ++j) {
				if (!is_connected[i][cg.adj[i][j]]) {
					is_connected[i][cg.adj[i][j]] = true;
				}
			}
		}
		for (auto i = 0; i < SCC_SIZE - 1; ++i) {
			for (auto j = i + 1; j < SCC_SIZE; ++j) {
...
int main()
{
	...
	g.add_edge(5, 7);
	g.add_edge(7, 7);
	if (!g.is_semiconnected()) {
		std::cout << "this graph is not semiconnected!\n";
	}
	else {
		std::cout << "this graph is semiconnected!\n";
	}
}
