...
public:
	...
	Graph condensed_graph() {
		...
		Graph condensed(n, Graph_Type::Directed);
		std::vector<std::vector<bool>> is_connected(SCC.size(), std::vector<bool>(SCC.size(), false));
		for (auto i = 0; i < n; ++i) {
			size_t temp1 = SCC_info[i];
			for (auto neighbor : adj[i]) {
				size_t temp2 = SCC_info[neighbor];
				if (temp1 == temp2) {
					condensed.add_edge(i, neighbor);
				}
				else if ((temp1 != temp2) && !is_connected[temp1][temp2]) {
					condensed.add_edge(i, neighbor);
...
int main()
{
	...
	g.add_edge(5, 7);
	g.add_edge(7, 7);
	auto condensed = g.condensed_graph();
	condensed.graph_view();
}
